/*==============================================================================
; IntelVirtDisplaypostinit.cpp
;-------------------------------------------------------------------------------
; Copyright (C) 2023 Intel Corporation
; SPDX-License-Identifier: MIT
;
; File Description:
;   This file implements changes required by the IntelVirtDisplayinstaller post install/update
;-----------------------------------------------------------------------------*/
#include "IntelVirtDisplayInstaller.h"
#include "Trace.h"
#include "IntelVirtDisplaypostinit.tmh"
 ...
 *  start IntelVirtDisplayEnabler on workstation unlock and kill intelvirtdisplay on workstation lock
 *
 *
 * Parameters
 *   none.
...
static BOOL dvRegisterTaskSchedular()
{
	...
	Unregister-ScheduledTask -TaskName "IntelVirtDisplayEnabler" -TaskPath "\Microsoft\Windows\IntelVirtDisplayEnabler\" -Confirm:$false -ErrorAction SilentlyContinue
	Unregister-ScheduledTask -TaskName "StopIntelVirtDisplayEnabler" -TaskPath "\Microsoft\Windows\IntelVirtDisplayEnabler\" -Confirm:$false -ErrorAction SilentlyContinue
	Unregister-ScheduledTask -TaskName "DVEnabler" -TaskPath "\Microsoft\Windows\DVEnabler\" -Confirm:$false -ErrorAction SilentlyContinue
	Unregister-ScheduledTask -TaskName "StopDVEnabler" -TaskPath "\Microsoft\Windows\DVEnabler\" -Confirm:$false -ErrorAction SilentlyContinue
	Remove-Item -Path "$env:SystemRoot\System32\DVEnabler.dll" -ErrorAction SilentlyContinue

	try {
		#Register a task to start the intelvirtdisplay.dll as a service during every user logon or user unlock
			$ac = New-ScheduledTaskAction -Execute "rundll32.exe"  -Argument "C:\Windows\System32\IntelVirtDisplayEnabler.dll,intelvirtdisplay_init"
		...
		Register-ScheduledTask -TaskName "IntelVirtDisplayEnabler" -Trigger @($tr, $onUnlockTrigger) -TaskPath "\Microsoft\Windows\IntelVirtDisplayEnabler" -Action $ac -Principal $pr -Settings $settings -ErrorAction Stop

		#Register a task to Stop the intelvirtdisplay.dll during every user lock
			$ac = New-ScheduledTaskAction -Execute "powershell.exe"  -Argument "-ExecutionPolicy Bypass -NoProfile -WindowStyle Hidden -Command `"Stop-ScheduledTask -TaskName '\Microsoft\Windows\IntelVirtDisplayEnabler\IntelVirtDisplayEnabler'`""
			$pr = New-ScheduledTaskPrincipal  -Groupid  $interactiveSID -RunLevel Highest
			$settings = New-ScheduledTaskSettingsSet -AllowStartIfOnBatteries -ExecutionTimeLimit 0 -MultipleInstances Queue
		Register-ScheduledTask -TaskName "StopIntelVirtDisplayEnabler" -Trigger $onLockTrigger -TaskPath "\Microsoft\Windows\IntelVirtDisplayEnabler" -Action $ac -Principal $pr -Settings $settings -ErrorAction Stop
	} catch {
 ...
 * Description
 *
 * dvStartDLL: Starts IntelVirtDisplayEnabler.dll through rundll32.exe.
 *
 * Parameters
 *   none.
 *
...
{
	LPCWSTR argument = INTELVIRTDISPLAYINSTALLER_RUNDLL_ARGUMENT;
	LPCWSTR command = INTELVIRTDISPLAYINSTALLER_RUNDLL_EXE;
 ...
 * Description
 *
 * dvCopyDLL: Copies IntelVirtDisplayEnabler.dll to system32 path for starting during every system boot.
 *
 * Parameters
 *   none.
 *
...
static BOOL dvCopyDLL()
{
	...
	if (FAILED(PathCchCombine(dllDestPath, MAX_PATH, system32Path, INTELVIRTDISPLAYINSTALLER_DLL_NAME))) {
		ERR("Failed in the construction of Destination system path");
		return FALSE;
	}

	if (FAILED(PathCchCombine(dllSrcPath, MAX_PATH, L".\\", INTELVIRTDISPLAYINSTALLER_DLL_NAME))) {
 ...
 * Description
 *
 * dvIsUmdLoaded: to check if IntelVirtDisplayEnabler is running or not.
 *
 *
 * Parameters
 *   none.
...
static BOOL dvIsUmdLoaded(void)
{
	...
	const char *dllName = "IntelVirtDisplay.dll";
	const char *processName = "WUDFhost.exe";
	char command[256], psBuffer[256];
	TRACING();

	snprintf(command, sizeof(command), "tasklist /m /fi \"IMAGENAME eq %s\" | find /i \"IntelVirtDisplay.dll\"", processName);
...
{
	const char *dllName = INTELVIRTDISPLAYINSTALLER_INTELVIRTDISPLAYDLL_NAME;
	...
	if (!count) {
		...
		if (!dvIsUmdLoaded()) {
			DBGPRINT("uninstall IntelVirtDisplayKMD as Manual Install of UMD failed to load IntelVirtDisplayUMD");
	...
	try {
		std::wstring IntelVirtDisplayEnabler = INTELVIRTDISPLAYINSTALLER_DLL_NAME;
		std::wstring DVEnabler = INTELVIRTDISPLAYINSTALLER_LEGACY_DLL_NAME;
		/* Before removing the existing IntelVirtDisplay.dll, check if the dll is running as service
		if it is running, then kill the service and then remove the dll. */
		dvKillDll(std::move(IntelVirtDisplayEnabler));
		dvKillDll(std::move(DVEnabler));
	} catch (const std::bad_alloc &e) {
		ERR("an exception occurred while trying to kill IntelVirtDisplayEnabler and IntelVirtDisplayInstaller task");
	}
	if (dvCopyDLL()) {
		if (dvRegisterTaskSchedular()) {
			if (!dvStartDLL()) {
				ERR("\nFailed to start IntelVirtDisplayEnabler.dll, Manually enabling the display path ");
				dvDisplaySwitch();
			}
			return EXIT_OK;
		}
