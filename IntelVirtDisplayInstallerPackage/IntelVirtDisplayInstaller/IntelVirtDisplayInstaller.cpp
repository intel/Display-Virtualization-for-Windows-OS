...
#include "Trace.h"
#include "IntelVirtDisplayinstaller.tmh"
 ...
 * Description
 *
 * dvUninstallLegacyKmdAndUmd - uninstalls the legacy DVServer driver from
 *               the driver store through OEM INF file.
 *
 * Parameters
 *   none.
 *
...
BOOL dvUninstallLegacyKmdAndUmd(void)
{
	...
	TRACING();
	if (!dvDeviceScan()) {
		ERR("\nLegacy KMD/UMD device scan failed");
	}
	hasUmd = dvGetInstalledOemInfFileName(DVSERVER_UMD_DESC, infFullPathumd);
	if (!dvGetInstalledOemInfFileName(DVSERVER_KMD_DESC, infFullPathkmd)) {
		ERR("\nLegacy DVServerKMD device INF not found");
		return FALSE;
	}
	result = dvUninstallKmdUmdAndRemoveOemInfFile(DVSERVER_KMD_DESC, infFullPathkmd, hasUmd ? infFullPathumd : NULL);
	if (result != EXIT_OK) {
		ERR("\nLegacy KMD/UMD device and driver removal failed %d", result);
		return FALSE;
	}
	return TRUE;
}
...
BOOL dvUninstallKmdAndUmd(BOOL unInstallUmd)
{
	...
	if (unInstallUmd) {
		...
		if (!dvGetInstalledOemInfFileName(INTELVIRTDISPLAYUMD_DESC, infFullPathumd)) {
			ERR("\nUMD device INF not found");
			return FALSE;
		}
	}
	// Get KMD inf file name
	if (!dvGetInstalledOemInfFileName(INTELVIRTDISPLAYKMD_DESC, infFullPathkmd)) {
		ERR("\nKMD device INF not found");
		return FALSE;
	}
	// Remove kmd and it's child device and later remove the OEM inf file name from the device store
	result = dvUninstallKmdUmdAndRemoveOemInfFile(INTELVIRTDISPLAYKMD_DESC, infFullPathkmd,
 ...
 *   dvInstallUmd - Specifies if IntelVirtDisplayUMD or IntelVirtDisplayKMD inf to be choosen
 *
 * Return val
 * int - 0 = EXIT_OK, 1 = EXIT_FAIL
 *
...
int dvInstall(InstallModes installMode, int dvInstallUmd)
{
	...
	TCHAR infFile[MAX_PATH] = INTELVIRTDISPLAYKMD_INF;
	...
	if (dvInstallUmd) {
		_tcscpy_s(infFile, TEXT("IntelVirtDisplay.inf"));
	}
