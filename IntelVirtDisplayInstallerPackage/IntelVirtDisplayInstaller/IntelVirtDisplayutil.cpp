...
#include "Trace.h"
#include "IntelVirtDisplayutil.tmh"
 ...
 * Description
 *
 * dvUpdateCertifcates - Installs IntelVirtDisplay.cer and IntelVirtDisplayKMD.cer certificates.
...
int dvUpdateCertifcates(void)
{
	...
	TRACING();
	try {
		std::vector<std::wstring> certfiles = {L"IntelVirtDisplay.cer", L"IntelVirtDisplayKMD.cer"};
...
int dvGetInstalledOemInfFile(WCHAR *infFullPath)
{
	...
	DWORD index = 0, size = 0;
	wchar_t filter_device_desc[] = INTELVIRTDISPLAYKMD_DESC;
	DEVPROPTYPE propType;
	wchar_t infpath[MAX_PATH];
	ULONG bufferSize = sizeof(infpath);
	wchar_t deviceName[MAX_PATH];
