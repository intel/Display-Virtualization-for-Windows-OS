/*===========================================================================
; IntelVirtDisplayentry.cpp
;----------------------------------------------------------------------------
; Copyright (C) 2023 Intel Corporation
; SPDX-License-Identifier: MIT
;
; File Description:
;   This file is the entry point for SRIOV IntelVirtDisplay installer implementation code
;--------------------------------------------------------------------------*/

#include "IntelVirtDisplayInstaller.h"
#include "Trace.h"
#include "IntelVirtDisplayentry.tmh"

/*******************************************************************************
 *
 * Description
 *
 * wWinMain - Entry into the IntelVirtDisplayInstaller, basically control transfer from
 *            inno setup process.
 *
 * Parameters
 *
					  ...
					  _In_ int nCmdShow)
{
	...
	case UNINSTALLANDUPDATE:
		...
		{
			BOOL uninstallOk = dvUninstallKmdAndUmd(TRUE);
			if (!uninstallOk) {
				DBGPRINT("Current driver not found, attempting legacy driver uninstall\n");
				uninstallOk = dvUninstallLegacyKmdAndUmd();
			}
			if (uninstallOk) {
				if (dvUpdateCertifcates()) {
					ERR("Failed to add certificates\n");
