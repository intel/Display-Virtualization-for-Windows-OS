...
void *mem_alloc_contiguous_pages(void *context, size_t size)
{
    ...
    if (ptr) {
        ...
        DbgPrintAdopt(TRACE_LEVEL_FATAL, ("Ran out of memory in alloc_pages_exact(%Id)\n", size));
    }
    return ptr;
}
    ...
    size_t length)
{
    ...
    if (BytesRead != length)
    {
        DbgPrintAdopt(TRACE_LEVEL_ERROR, ("[%s] read %d bytes at %d\n", __FUNCTION__, BytesRead, where));
        return -1;
    }
    return 0;
}
    ...
    if (m_BaseVA == nullptr)
    {
        if (m_bPortSpace)
        {
            if (m_bIoMapped)
            {
                ...
                {
                    DbgPrintAdopt(TRACE_LEVEL_VERBOSE, ("[%s] mapped port BAR at %x\n", __FUNCTION__, m_BasePA.LowPart));
                }
                else
                {
                    m_BaseVA = nullptr;
                    DbgPrintAdopt(TRACE_LEVEL_ERROR, ("DxgkCbMapMemor (CmResourceTypePort) failed with status 0x%X\n", Status));
                }
            }
            else
            {
        ...
        else
        {
            ...
            if (m_BaseVA)
            {
                DbgPrintAdopt(TRACE_LEVEL_VERBOSE, ("[%s] mapped memory BAR at %I64x\n", __FUNCTION__, m_BasePA.QuadPart));
            }
            else
            {
                m_BaseVA = nullptr;
                DbgPrintAdopt(TRACE_LEVEL_ERROR, ("[%s] failed to map memory BAR at %I64x\n", __FUNCTION__, m_BasePA.QuadPart));
            }
        }
    }
    return m_BaseVA;
...
bool CPciResources::Init(PVOID pvDeviceContext, PVOID pResList)
{
    ...
    DbgPrintAdopt(TRACE_LEVEL_VERBOSE, ("<--> %s BytesRead = %d\n", __FUNCTION__, BytesRead));

    if (BytesRead != sizeof(pci_config)) {
        DbgPrintAdopt(TRACE_LEVEL_ERROR, ("[%s] could not read PCI config space\n", __FUNCTION__));
        return false;
    }

    DbgPrintAdopt(TRACE_LEVEL_VERBOSE, ("---> %s ListCount = %d\n", __FUNCTION__, pResList->Count));

    for (ULONG i = 0; i < WdfCmResourceListGetCount((WDFCMRESLIST)pResList); i++) {

        pResDescriptor = WdfCmResourceListGetDescriptor((WDFCMRESLIST)pResList, i);

        if (!pResDescriptor) {
            DbgPrintAdopt(TRACE_LEVEL_ERROR, ("[%s] could not get resource descriptor.\n", __FUNCTION__));
            return false;
        }
        ...
        switch (pResDescriptor->Type)
        {
            ...
            {
                DbgPrintAdopt(TRACE_LEVEL_FATAL, ("CmResourceTypePort\n"));
                break;
            }
            break;
            case CmResourceTypeInterrupt:
            {
                ...
                if (IsMSIEnabled())
                {
                    DbgPrintAdopt(TRACE_LEVEL_FATAL, ("Found MSI Interrupt vector %d, level %d, affinity 0x%X, flags %X\n",
                ...
                {
                    DbgPrintAdopt(TRACE_LEVEL_FATAL, ("Found Interrupt vector %d, level %d, affinity 0x%X, flags %X\n",
            ...
            case CmResourceTypeMemory:
            {
                ...
                DbgPrintAdopt(TRACE_LEVEL_FATAL, ("Found IO memory at %08I64X(%d) bar %d\n", Start.QuadPart, len, bar));
                if (bar < 0)
                {
                    break;
                }
            ...
            case CmResourceTypeDma:
                DbgPrintAdopt(TRACE_LEVEL_FATAL, ("Dma\n"));
                break;
            case CmResourceTypeDeviceSpecific:
                DbgPrintAdopt(TRACE_LEVEL_FATAL, ("Device Specific\n"));
                break;
            case CmResourceTypeBusNumber:
                DbgPrintAdopt(TRACE_LEVEL_FATAL, ("Bus number\n"));
                break;
            default:
                DbgPrintAdopt(TRACE_LEVEL_ERROR, ("Unsupported descriptor type = %d\n", pResDescriptor->Type));
                break;
        }
    }
    ...
    {
        DbgPrintAdopt(TRACE_LEVEL_FATAL, ("[%s] resource enumeration failed\n", __FUNCTION__));
        return false;
    }
...
PVOID CPciResources::GetMappedAddress(UINT bar, ULONG uOffset)
{
    ...
    {
        DbgPrintAdopt(TRACE_LEVEL_ERROR, ("[%s] failed to map BAR %d, offset %x\n", __FUNCTION__, bar, uOffset));
        return nullptr;
    }
}
    ...
    _Outptr_result_bytebuffer_(Length) VOID**      VirtualAddress)
{
    ...
    if ((PhysicalAddress.QuadPart == (ULONGLONG)0) ||
        ...
        (VirtualAddress == NULL))
    {
        DbgPrintAdopt(TRACE_LEVEL_ERROR, ("One of PhysicalAddress.QuadPart (0x%I64x), Length (%lu), VirtualAddress (%p) is NULL or 0\n",
    ...
    *VirtualAddress = MmMapIoSpace(PhysicalAddress,
    ...
    if (*VirtualAddress == NULL)
    {
        ...
        *VirtualAddress = MmMapIoSpace(PhysicalAddress,
        ...
        if (*VirtualAddress == NULL)
        {
            DbgPrintAdopt(TRACE_LEVEL_ERROR, ("MmMapIoSpace returned a NULL buffer when trying to allocate %lu bytes", Length));
            return STATUS_NO_MEMORY;
        }
    }

    DbgPrintAdopt(TRACE_LEVEL_FATAL, ("%s PhysicalAddress.QuadPart (0x%I64x), Length (%lu), VirtualAddress (%p)\n",
    ...
    _In_                ULONG Length)
{
    ...
    DbgPrintAdopt(TRACE_LEVEL_FATAL, ("%s Length (%lu), VirtualAddress (%p)\n",
    ...
    {
        DbgPrintAdopt(TRACE_LEVEL_ERROR, ("Only one of Length (%lu), VirtualAddress (%p) is NULL or 0",
