...
BYTE *GetRowStart(_In_ CONST BLT_INFO *pBltInfo, CONST RECT *pRect)
{
	...
	case D3DKMDT_VPPR_ROTATE90:
		...
		break;
	case D3DKMDT_VPPR_ROTATE180:
		pRet = ((BYTE *)pBltInfo->pBits + (pBltInfo->Height - 1 - OffTop) * pBltInfo->Pitch +
...
VOID CopyBitsGeneric(BLT_INFO *pDst, CONST BLT_INFO *pSrc, UINT NumRects, _In_reads_(NumRects) CONST RECT *pRects)
{
	...
	DBGPRINT("NumRects = %d Dst = %p Src = %p\n", NumRects, pDst->pBits, pSrc->pBits);
...
#pragma prefast(                                                                                                       \
	...
	__except (EXCEPTION_EXECUTE_HANDLER) {
		ERR("Either dst (0x%p) or src (0x%p) bits encountered exception during access.\n", pDst->pBits, pSrc->pBits);
	}
}
