...
)
{
	...
	set_module_name("DVServerUMD_Node");
...
int __cdecl main(int argc, wchar_t* argv[])
{
	...
	if (FAILED(hr))
	{
		ERR("DVServerUMD_Node : SwDeviceCreate failed\n");
		return 1;
	}

	// Wait for callback to signal that the device has been created
	PRINT("DVServerUMD_Node : Waiting for device to be created....\n");
	DWORD waitResult = WaitForSingleObject(hEvent, 10 * 1000);
	if (waitResult != WAIT_OBJECT_0)
	{
		ERR("DVServerUMD_Node : Wait for device creation failed\n");
		SwDeviceClose(hSwDevice);
		return 1;
	}
	PRINT("DVServerUMD_Node : Device created\n");
