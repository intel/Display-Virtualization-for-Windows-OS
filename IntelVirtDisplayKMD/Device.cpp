...
#ifdef ALLOC_PRAGMA
#pragma alloc_text(PAGE, IntelVirtDisplayKMDCreateDevice)
#endif
...
--*/
{
	...
	if (NT_SUCCESS(status)) {
		...
		//
		// Create the IntelVirtDisplayKMD device instance
		//
		pDeviceContext->pvDeviceExtension = NULL; // allocated in EvtPrepareHardware

		//
		// Create a device interface so that applications can find and talk
		// to us.
		//
		status = WdfDeviceCreateDeviceInterface(device, &GUID_DEVINTERFACE_IntelVirtDisplayKMD,
		...
		if (NT_SUCCESS(status)) {
			...
			//
			status = IntelVirtDisplayKMDQueueInitialize(device);
		}
		...
		if (NT_SUCCESS(status)) {
			...
			WDF_INTERRUPT_CONFIG_INIT(&interruptConfig, IntelVirtDisplayKMDEvtInterruptISR, IntelVirtDisplayKMDEvtInterruptDPC);

			//
			// These first two callbacks will be called at DIRQL. Their job is to
			// enable and disable interrupts.
			//
			interruptConfig.EvtInterruptEnable = IntelVirtDisplayKMDEvtInterruptEnable;
			interruptConfig.EvtInterruptDisable = IntelVirtDisplayKMDEvtInterruptDisable;
...
NTSTATUS IntelVirtDisplayKMDEvtPrepareHardware(_In_ WDFDEVICE Device, _In_ WDFCMRESLIST ResourcesRaw,
...
--*/
{
    ...
    //
	// Allocate the VioGpuAdapterLite instance here so that it is always
	// re-created on PnP resource rebalance (paired with deletion in
	// EvtReleaseHardware).
	//
	...
	//
	// TODO:
	//  The following display info will be passed down from IntelVirtDisplayUMD
	//  For now, just use hard coded one
	//
	status = pVioGpuAdapterLite->HWInit(ResourcesTranslated, &pVioGpuAdapterLite->DisplayInfo);

	if (!NT_SUCCESS(status)) {
		delete pVioGpuAdapterLite;
		pDeviceContext->pvDeviceExtension = NULL;
	}

	return status;
}

NTSTATUS IntelVirtDisplayKMDEvtReleaseHardware(_In_ WDFDEVICE Device, _In_ WDFCMRESLIST ResourcesTranslated)
/*++
...
--*/
{
	...
	pDeviceContext->pvDeviceExtension = NULL; // prevent stale pointer reuse 
	return status;
}

NTSTATUS IntelVirtDisplayKMDEvtD0Entry(_In_ WDFDEVICE Device, _In_ WDF_POWER_DEVICE_STATE PreviousState)
/*++
...
Routine Description:
	...
	IntelVirtDisplayKMDEvtDeviceD0EntryPostInterruptsEnabled, which will ensure
...
NTSTATUS IntelVirtDisplayKMDEvtD0Exit(_In_ WDFDEVICE Device, _In_ WDF_POWER_DEVICE_STATE PreviousState)
/*++
...
--*/
{
	...
	//
	// Retrieve the IntelVirtDisplayKMD device instance
	//
	pVioGpuAdapterLite = (VioGpuAdapterLite *)pDeviceContext->pvDeviceExtension;
...
BOOLEAN IntelVirtDisplayKMDEvtInterruptISR(_In_ WDFINTERRUPT Interrupt, _In_ ULONG MessageID)
/*++
...
void IntelVirtDisplayKMDEvtInterruptDPC(_In_ WDFINTERRUPT Interrupt, _In_ WDFOBJECT AssociatedObject)
/*++
...
NTSTATUS
IntelVirtDisplayKMDEvtDeviceD0ExitPreInterruptsDisabled(IN WDFDEVICE Device, IN WDF_POWER_DEVICE_STATE TargetState)
/*++
...
NTSTATUS
IntelVirtDisplayKMDEvtDeviceD0EntryPostInterruptsEnabled(IN WDFDEVICE Device, IN WDF_POWER_DEVICE_STATE PreviousState)
/*++

Routine Description:

	IntelVirtDisplayKMDEvtDeviceD0EntryPostInterruptsEnabled is called by the framework after the
...
NTSTATUS IntelVirtDisplayKMDEvtInterruptEnable(IN WDFINTERRUPT Interrupt, IN WDFDEVICE AssociatedDevice)
/*++
...
NTSTATUS IntelVirtDisplayKMDEvtInterruptDisable(IN WDFINTERRUPT Interrupt, IN WDFDEVICE AssociatedDevice)
/*++
